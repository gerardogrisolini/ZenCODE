...
- ACP agent over stdio: connects to compatible clients including **Xcode 27** as a native coding agent.
...
- Saved sessions with checkpoint trees: per-project snapshots with branching, checkpoints, and in-place restore from any point; per-project graph-backed memory with automatic recall — offline BM25 keyword retrieval plus graph expansion, no second LLM call, injected into every round of the turn as a character-budgeted block (about 1k tokens by default) — and optional semantic search via an opt-in embeddings endpoint; automatic extraction is opt-in and off by default; plus `AGENTS.md` for durable guidance.
