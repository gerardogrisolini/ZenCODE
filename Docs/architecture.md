...
| `Sources/ZenCODECore/ZenCODE/Telegram` | Telegram remote-control runtime: Bot API transport, long-polling, pairing, channel state, voice-attachment download, tool-call permission brokering, safe tool-call presentation, and the ordered per-turn progress reporter. `TerminalTelegramControlService` retains lifecycle and ingress coordination; pairing, wire effects, attachments, and the command registry are separated into dedicated runtime files. It depends only on `Foundation`/`FoundationNetworking`, `ToolCore`, and neutral runtime contracts (`Agent` settings, `Agent` tool-authorization requests, `ToolCallPresentation`, voice input); it must not depend on terminal presentation types. The `TerminalChat+Telegram*.swift` extensions under `ZenCODETUI/Chat/Telegram` are the only adapter that binds this runtime to input, rendering, and prompt-queue surfaces. |
...
Durable memory storage, retrieval, embedding, recall, and the persisted graph
format are part of this contract and are specified in
[memory.md](memory.md). `ZenCODECore` owns the `ZenMemory` engine and its public
`Memory` facade; the per-workspace graph file under the support directory is the
authoritative store, and its format changes only through an explicit migration.
