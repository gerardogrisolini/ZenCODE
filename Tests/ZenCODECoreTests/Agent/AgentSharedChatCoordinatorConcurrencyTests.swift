...
struct AgentSharedChatCoordinatorConcurrencyTests {
    ...
    // MARK: - Deprecated room-scoped API

    /// The removed room-scoped spelling still compiles and still coordinates,
    /// and it grants at most one turn: the compatibility layer routes a legacy
    /// claim through a real observer identity instead of a room-wide back door.
    @available(*, deprecated, message: "Exercises the deprecated room-scoped API on purpose.")
    @Test
    func deprecatedRoomScopedAPIStillClaimsExactlyOneTurn() async throws {
        let room = Self.makeRoomID()
        let source = TestSharedChatSource()
        await source.enqueue([Self.message("legacy consumer", roomID: room)])
        let coordinator = Self.makeCoordinator(source: source)
        let events = await coordinator.observe(roomID: room)
        ...
        // The second answer to the same broadcast is a no-op, exactly like the
        // observation-based path.
        #expect(await coordinator.resolveAutoTrigger(
            id: triggerID,
            roomID: room,
            resolution: .started
        ...
        // The legacy stream is a real observation: it carries the same events.
        var received: [AgentSharedChatCoordinatorEvent] = []
        await coordinator.stopAll()
        for await event in events {
            received.append(event)
        }
        #expect(received.contains { event in
            if case .autoTrigger(let trigger) = event { return trigger.id == triggerID }
            return false
        })
    }

    /// Legacy busy state and legacy declines keep their meaning: the batch is
    /// preserved and the room stays closed while the consumer says it is busy.
    @available(*, deprecated, message: "Exercises the deprecated room-scoped API on purpose.")
    @Test
    func deprecatedRoomScopedBusyStateAndDeclineStillPreserveTheBatch() async throws {
        let room = Self.makeRoomID()
        let source = TestSharedChatSource()
        await source.enqueue([Self.message("legacy busy", roomID: room)])
        let coordinator = Self.makeCoordinator(source: source)
        let events = await coordinator.observe(roomID: room)
        ...
        #expect(await coordinator.claimedAutoTriggerOwnerID(roomID: room) == nil)
        // Declining re-queues; the room immediately offers the batch again.
        let reofferedID = try #require(await coordinator.activeAutoTriggerID(roomID: room))
        #expect(reofferedID != triggerID)
        #expect(await coordinator.activeAutoTriggerMessageCount(roomID: room) == 1)
    ...
    /// Without a legacy observation a claim has no owner, so the compatibility
    /// layer reports `notAcquired` instead of granting an unattributable turn —
    /// while a decline, which only ever returns work, still applies.
    @available(*, deprecated, message: "Exercises the deprecated room-scoped API on purpose.")
    @Test
    func deprecatedClaimWithoutALegacyObservationIsRefused() async throws {
        let room = Self.makeRoomID()
        let source = TestSharedChatSource()
        await source.enqueue([Self.message("no legacy observer", roomID: room)])
        let coordinator = Self.makeCoordinator(source: source)
        // Attached through the supported API: the legacy calls own no identity.
        let subscription = await coordinator.observeSubscription(roomID: room)
        ...
        #expect(await coordinator.claimedAutoTriggerOwnerID(roomID: room) == nil)
        // The supported owner can still take it: the refusal changed nothing.
        #expect(await coordinator.resolveAutoTrigger(
            id: triggerID,
            observation: subscription,
            resolution: .started
    ...
    /// The runner keeps the removed room-scoped entry points callable and
    /// forwards them to the same coordinator.
    @available(*, deprecated, message: "Exercises the deprecated room-scoped API on purpose.")
    @Test
    func deprecatedSessionRunnerSharedChatAPIRemainsCallable() async throws {
        let sessionID = "session-\(UUID().uuidString)"
        let runner = AgentCoreSessionRunner()

        let events = await runner.observeSharedChat(rootSessionID: sessionID)
        await runner.setSharedChatConsumerBusy(true, rootSessionID: sessionID)
        await runner.setSharedChatConsumerBusy(false, rootSessionID: sessionID)
        // No trigger was ever published, so a legacy resolution is a no-op.
        #expect(await runner.resolveSharedChatAutoTrigger(
            id: UUID(),
        ...
        // Stopping the room ends the legacy stream instead of leaking it.
