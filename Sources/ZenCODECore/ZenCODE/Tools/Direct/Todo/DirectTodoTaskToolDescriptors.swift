...
extension DirectTodoRuntime {
    ...
    private static let taskUpdateDescriptor = DirectToolDescriptor(
            name: "tasks.update",
            description: "Updates task metadata, progress, result, evidence, or an allowed lifecycle transition. Use tasks.retry and tasks.cancel for those operations. Alternatively the root coordinator may update /goal workflow control with only graphID, expectedRevision (from tasks.list), and workflow {state: running|awaiting_user|blocked, message}. This mode requires an active current workflow graph and forbids every task field; originalGoal is immutable. Paused states require a message. Text headings do not suspend a workflow.",
            inputSchema: #"{"type":"object","properties":{"id":{"type":"string"},"taskID":{"type":"string"},"task_id":{"type":"string"},"graphID":{"type":"string"},"graph_id":{"type":"string"},"title":{"type":"string"},"name":{"type":"string"},"details":{"type":["string","null"]},"description":{"type":["string","null"]},"status":{"type":"string"},"statusReason":{"type":"string"},"status_reason":{"type":"string"},"priority":{"type":"string"},"complexity":{"type":"integer","minimum":1,"maximum":10,"description":"Task difficulty 1-10. \#(TaskRecord.complexityRubric). Agent selection policy: \#(TaskRecord.agentSelectionPolicy)"},"dependsOn":{"type":"array","items":{"type":"string"}},"depends_on":{"type":"array","items":{"type":"string"}},"output":{"type":"string"},"progress":{"type":"string"},"error":{"type":"string"},"workflow":{"type":"object","properties":{"state":{"type":"string","enum":["running","awaiting_user","blocked"]},"message":{"type":"string"}},"required":["state"],"additionalProperties":false},"evidence":{"type":"array","items":{}},"expectedRevision":{"type":"integer"},"expected_revision":{"type":"integer"}},"anyOf":[{"required":["id"]},{"required":["graphID","expectedRevision","workflow"]}]}"#,
    ...
    private static let taskRetryDescriptor = DirectToolDescriptor(
            name: "tasks.retry",
            description: "Retries a failed or blocked task while preserving all prior attempts and outputs. A retried /goal task must be claimed through a new canonical agent.create agents item containing `taskID`; do not reopen its completed attempt with agent.message.",
