...
extension DirectSubAgentRuntime {
    ...
    private static let agentCreateItemSchema = #"{"type":"object","properties":{"profile":{"type":"string","description":"Exact profile name from the delegatable roster."},"model":{"type":"string","description":"Exact binding:... reference from the selected profile when it has configured bindings. Otherwise omit model to inherit the main session model."},"taskID":{"type":"string","description":"Task ID returned by tasks.create/tasks.list. Put it in this agents item, never at the agent.create root."},"prompt":{"type":"string"},"name":{"type":"string"},"role":{"type":"string"}},"required":["profile"],"additionalProperties":false}"#
    ...
    private static let createDescriptor = DirectToolDescriptor(
            name: "agent.create",
            description: "Creates 1–8 delegated sub-agents from the exact profile and binding references in the delegatable roster. Use only the canonical agents array; each item requires profile. If the profile has configured bindings, model is required; otherwise omit model to inherit the main session model and provider. For graph work, put the task ID returned by tasks.create/tasks.list in that item's taskID field; taskID is never a root argument. Independent items run in parallel. A task-bound item claims its task atomically, and the child receives tasks.list, tasks.get, and tasks.update for attempt reporting. The selected profile owns the child tool grant. Legacy root fields and aliases remain input-compatible but are intentionally not model-visible.",
    ...
    private static let messageDescriptor = DirectToolDescriptor(
            name: "agent.message",
            description: "Sends a live shared-chat message. Use id/name/ids for direct delivery, or set `to` to operator, coordinator, peers, or all. `operator` replies directly to the human terminal operator; `all` broadcasts to the operator, coordinator, and every active agent. `target` remains a compatibility alias. Direct messages wake idle or standby recipients immediately. After completing your task attempt you remain in standby while the task graph is active: you can receive and reply to messages from the coordinator and peers. For code corrections after negative validation, use tasks.retry then a new canonical agent.create item containing `taskID` instead of messaging.",
            inputSchema: #"{"type":"object","properties":{"id":{"type":"string"},"name":{"type":"string"},"task_id":{"type":"string"},"ids":{"type":"array","items":{"type":"string"}},"to":{"type":"string","enum":["direct","operator","coordinator","peers","all"]},"target":{"type":"string","enum":["direct","operator","coordinator","peers","all"],"description":"Compatibility alias for to."},"message":{"type":"string"},"prompt":{"type":"string"},"input":{"type":"string"}},"required":["message"]}"#,
            presentation: ToolPresentationDefinition(
                title: "Agent",
                action: "Message",
                kind: .communicate,
