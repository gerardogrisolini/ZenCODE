...
struct WebSearchTool: FeatureTool {
    ...
    func run(_ input: Input, context: FeatureContext) async throws -> String {
        ...
        let session = WebToolsSupport.makeURLSession(
            policy: WebNetworkRequestPolicy(environment: context.environment)
        )
        let (data, response) = try await WebToolsSupport.fetchWithRetry(url: url, session: session)
...
struct WebFetchTool: FeatureTool {
    ...
    static let description = "Opens an HTTP or HTTPS URL restricted to public network destinations. On Apple platforms it renders the page in a silent in-process WebKit view (JavaScript executed) and returns extracted Markdown; on other platforms it falls back to a raw HTTP fetch preview. Private, loopback, link-local, and otherwise non-public destinations are rejected, including redirect chains that resolve to them."
    static let inputSchema = buildInputSchema(
    ...
    func run(_ input: Input, context: FeatureContext) async throws -> String {
        ...
        // One policy validates the initial URL and every redirect the fetch
        // later follows, so an untrusted public page cannot pivot into a
        // private network or metadata endpoint mid-flight.
        let requestPolicy = WebNetworkRequestPolicy(environment: context.environment)
        try requestPolicy.validateRequestURL(rawURL)
        ...
        #if canImport(WebKit)
        let page = try await WebKitPageRenderer.render(
            url: url,
            timeout: timeout,
...
public enum WebToolsFeatureRunner {
...
final class WebKitPageRenderer: NSObject, WKNavigationDelegate {
    ...
    private let requestPolicy: WebNetworkRequestPolicy
    private var loadContinuation: CheckedContinuation<Void, Error>?
    private var didResume = false
    private var timeoutTask: Task<Void, Never>?

    private init(requestPolicy: WebNetworkRequestPolicy) {
    ...
    static func render(
        ...
        requestPolicy: WebNetworkRequestPolicy
    ) async throws -> RenderedPage {
        let renderer = WebKitPageRenderer(requestPolicy: requestPolicy)
    ...
    /// Server and client navigations must pass the same public-network policy
    /// as the initial request. This is what stops a public page from
    /// redirecting the main frame into loopback or a metadata endpoint.
    nonisolated func webView(
        ...
        Task { @MainActor in
            do {
                // Server redirects of the main frame, and top-level client
                // navigations such as location assignments, both arrive here
                // before the document at the new URL is committed.
                guard navigationAction.targetFrame?.isMainFrame == true else {
                    decisionHandler(.allow)
                    return
                }
                try await requestPolicy.validateRequestURLOffloaded(
...
enum WebToolsSupport {
    ...
    /// A URLSession whose redirect handling is guarded by the network request
    /// policy, mirroring the interception approach of the Browser feature.
    /// Each hop (including the very first request, validated by the caller) is
    /// re-checked against URL restrictions and resolved addresses, so DNS
    ...
    static func rawFetchPreview(
        ...
        // The guard must fail closed on the initial request as well, not only
        // on the redirects the session-level delegate observes.
        try policy.validateRequestURL(url.absoluteString)

        var request = URLRequest(url: url)
        request.timeoutInterval = timeout
        request.setValue("ZenCODE/1.0", forHTTPHeaderField: "User-Agent")

        let session = makeURLSession(policy: policy)
